...
$login = autorize();
	...
	if ($login['soft']!==$HTTP_USER_AGENT || $login['ip']!==$REMOTE_ADDR){
	 if ($login["safe"]==1){
	  print '<card id="main" title="Zabrana" ontimer="room.php?id='.$id.'&amp;pass='.$pass.'&amp;room='.$room.'&amp;mod=$mod&amp;r='.$r.'"><timer value="300"/><p align="center">';
		echo "<big><b>Pristup na ovaj nick nije dozvoljen sa vasim modelom telefona!</b>";
		echo "</big></p></card></wml>";
		exit;
	 } 
	 else {
	  	  print '<card id="main" title="Security-Alert" ontimer="room.php?id='.$id.'&amp;pass='.$pass.'&amp;room='.$room.'&amp;mod=$mod&amp;r='.$r.'"><timer value="300"/><p align="center">';
	...
	echo '<a href="room.php?id='.$id.'&amp;pass='.$pass.'&amp;room='.$room.'&amp;r='.$r.'">OK</a></p></card></wml>';  		
...
if($login) { //если комната сущестрвует
	...
	if($room=="room1") 
		if ($moder < 1) {
				  //print '<card title="Zabrana!"><p align="center">';
		echo "<big><b>Nije vam dozvoljen pristup u ovu sobu!</b>";
		echo "</big></p></card></wml>";
		exit;
		...
		if ($login['posts'] < 1000 ) {
				  //print '<card title="Zabrana!"><p align="center">';
		echo "<big><b>Nije vam dozvoljen pristup u ovu sobu!</b></big><br/>Potrebno je da imate 1 000 poruka";
		echo "</p></card></wml>";
		exit;
	...
	if($room=="vict")
	@mysql_query("insert into `".$px.$vtable."` values(0,'".$login['login']."','$msg','".$pr[0]."','".$pr[1]."','','','".$timestp."');");
	else
	@mysql_query("insert into `".$px.$mtable."` values(0,'".$login['login']."','$msg','".$pr[0]."','".$pr[1]."','".$timestp."','$room');");
	//обновление постов пользователя
	@mysql_query("update `".$px.$utable."` set `posts`='".++$posts."', `ltime`='".$timestp."' where `id`='".$id."';"); } 
