...
'<p>';
	...
	while($droom = @mysql_fetch_array($q)) {
		$q_online = @mysql_query("SELECT `id`,`login`, `room`, `moder` FROM `".$px.$utable."` WHERE ltime>'".intval(time()-$offline)."' AND room='".$droom['var']."' order by ltime desc;");
		print '<b><a href="./room.php?id='.$id.'&amp;pass='.$pass.'&amp;room='.$droom['var'].'">'.$droom['val1'].'</a></b>:<br/>';
		while($donline = @mysql_fetch_array($q_online)) if ($login['admin'] = 1 && $donline['moder'] != 3 && $donline['moder'] != "-1")print "<a href=\"./user.php?id=$id&amp;pass=$pass&amp;room=".$donline['room']."&amp;dbid=".$donline['id']."\">".$donline['login']."</a><br/>"; 
//		print '--------';
	}
