...
if($login&&$login['admin']) {
	...
	if(empty($act)) {
	print $lang['title']."<br/>
	<anchor>ON<go href=\"$PHP_SELF?act=update&amp;id=$id&amp;pass=$pass&amp;mod=$mod\" method=\"post\"><postfield name=\"name\" value=\"$(name)\"/><postfield name=\"t\" value=\"on\"/></go></anchor>
	<anchor>OFF<go href=\"$PHP_SELF?act=update&amp;id=$id&amp;pass=$pass&amp;mod=$mod\" method=\"post\"><postfield name=\"name\" value=\"$(name)\"/><postfield name=\"t\" value=\"off\"/></go></anchor>
	<br/>";
	} else {
	//$t=htmlspecialchars(stripslashes(trim(substr($t,0,50))));
	if(@mysql_query("update `".$px.$stable."` set `val1`='".$login['login']."',`val2`='$t',`val3`='".time()."'  where `mod`='reg';")) print $lang['done'];
	}
	break;
	
	case 'info':
	if(empty($act)) {
	print $lang['title']."<br/><input type=\"text\" name=\"t\"/><br/>
	<anchor>".$lang['ok']."<go href=\"$PHP_SELF?act=update&amp;id=$id&amp;pass=$pass&amp;mod=$mod\" method=\"post\"><postfield name=\"name\" value=\"$(name)\"/><postfield name=\"t\" value=\"$(t)\"/></go></anchor><br/>";
	} else {
	//$t=htmlspecialchars(stripslashes(trim(substr($t,0,50))));
	if(@mysql_query("update `".$px.$stable."` set `val1`='".$login['login']."',`val2`='$t',`val3`='".time()."'  where `mod`='info';")) print $lang['done'];
	}
	break;
	...
	print "<a href=\"$PHP_SELF?id=$id&amp;pass=$pass&amp;mod=info\">Razglas</a><br/>";
	print "<a href=\"$PHP_SELF?id=$id&amp;pass=$pass&amp;mod=regoff\">Registracija</a><br/>";
