...
if(!$search){ 
	...
	if ($data['room'] != $login['room']) {
 	...
 	if ($data['ltime']>intval(time()-$offline)){ 
	$qroom = @mysql_query("select * from `".$px.$stable."` where var='".$data['room']."';");
	$droom = @mysql_fetch_array($qroom);
	print "Online je! <br/>"; 
		if ($data['room']) print $lang['room'].": <a href=\"room.php?id=$id&amp;pass=$pass&amp;room=".$data['room']."&amp;r=$r\">".$droom['val1']."</a>";
		else print "<a href=\"room.php?id=$id&amp;pass=$pass&amp;room=$room&amp;r=$r\">".$lang['to_chat']."</a><br/>" ;
	}else print "Trenutno je offline";
	
}

print "<br/><a href=\"$PHP_SELF?id=$id&amp;pass=$pass&amp;dbid=$prid&amp;room=$room&amp;mod=profil\">".$lang['profile']."</a><br/>";

} else {
print"<card id=\"prof\" title=\"".$lang['profile']."\"><p>";
...
if($user_moder=="-1") { print "<b>:::</b><br/><a href=\"admin.php?id=$id&amp;pass=$pass&amp;whoid=".$data['id']."&amp;room=$room&amp;mod=delnevidljiv\">Iskljuci Nevidljivost</a><br/>"; }
else {
print "<b>:::</b><br/><a href=\"admin.php?id=$id&amp;pass=$pass&amp;whoid=".$data['id']."&amp;room=$room&amp;mod=nevidljiv\">Ukljuci Nevidljivost</a><br/>"; }

if($user_moder>=1) { print "<a href=\"admin.php?id=$id&amp;pass=$pass&amp;whoid=".$data['id']."&amp;room=$room&amp;mod=delmoder\">".$lang['delmoder']."</a><br/>"; }
else {
print "<a href=\"admin.php?id=$id&amp;pass=$pass&amp;whoid=".$data['id']."&amp;room=$room&amp;mod=makemoder\">".$lang['makemoder']."</a><br/>"; }
