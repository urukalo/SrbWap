...
'<p>';
	...
	while($droom = @mysql_fetch_array($q)) {
		...
		while($donline = @mysql_fetch_array($q_online)) if ($donline['moder'] != 3 && $donline['moder'] != "-1")print "<a href=\"./user.php?id=$id&amp;pass=$pass&amp;room=".$donline['room']."&amp;dbid=".$donline['id']."\">".$donline['login']."</a><br/>"; 
//		print '--------';
	}
