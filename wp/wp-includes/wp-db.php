...
'/*/WP_I18N_DB_CONN_ERROR*/, $dbhost), 'db_connect_fail');
	...
	function set_prefix($prefix) {
		...
		if ( preg_match('|[^a-z0-9_]|i', $prefix) )
			return new WP_Error('invalid_db_prefix', /*WP_I18N_DB_BAD_PREFIX*/'Неисправан префикс базе података'/*/WP_I18N_DB_BAD_PREFIX*/);
...
<h1>Није могуће изабрати базу</h1>
<p>Успели смо да успоставимо везу са сервером базе (што значи да су ваше корисничко име и лозинка у реду) али нисмо успели да изаберемо базу <code>%1$s</code>.</p>
<ul>
<li>Да ли сте сигурни да постоји?</li>
<li>Да ли корисник <code>%2$s</code> има дозволу да користи базу <code>%1$s</code>?</li>
<li>На неким системима име ваше базе има префикс вашег корисничког, па би изгледао као <code>korisnicko_%1$s</code>. Да ли то може бити проблем?</li>
</ul>
<p>Уколико не знате како да подесите базу, требало би да <strong>контактирате вашег домаћина сервера</strong>. Уколико све остало не успе, можете потражити помоћ на <a href="http://sr.forums.wordpress.org/">Вордпресовим форумима за подршку</a>.</p>'/*/WP_I18N_DB_SELECT_DB*/, $db, $this->dbuser), 'db_select_fail');
			return;
		}
	}
	...
	function print_error($str = '') {
		...
		if ( $caller = $this->get_caller() )
			$error_str = sprintf(/*WP_I18N_DB_QUERY_ERROR_FULL*/'Грешка Вордпрес базе %1$s за упит %2$s направљен од %3$s'/*/WP_I18N_DB_QUERY_ERROR_FULL*/, $str, $this->last_query, $caller);
		else
			$error_str = sprintf(/*WP_I18N_DB_QUERY_ERROR*/'Грешка Вордпрес базе %1$s за упит %2$s'/*/WP_I18N_DB_QUERY_ERROR*/, $str, $this->last_query);
	...
	function get_row($query = null, $output = OBJECT, $y = 0) {
		...
		if ( $output == OBJECT ) {
			...
			$this->print_error(/*WP_I18N_DB_GETROW_ERROR*/' $db->get_row(string query, output type, int offset) -- Излаз мора бити један од: OBJECT, ARRAY_A, ARRAY_N'/*/WP_I18N_DB_GETROW_ERROR*/);
		}
	}
