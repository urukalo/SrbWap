...
//echo "LEVEL:$level:".$profildata['status'].":".$userdata['user_id'];
    ...
    if ($profildata['status']=="login"){
      if ($level!=$userdata['user_id'] || !isset($level)) $aSql="insert into profil ( id, loginid, tmp, tmp1, tmp2, tema, ikonice, `lastlogin`) values ( 'NULL','".$userdata['user_id']."','".$profildata['put']."','$jezik','$k','$ntmplate','standard','standardne', NOW() );";
        ...
        elseif ($profildata['status']=="update") $aSql="UPDATE profil SET tmplate = '".$profildata['tmplate']."',tema = '".$profildata['tema']."', ikonice = '".$profildata['ikonice']."', tmp = '".$profildata['tmp']."',tmp1 = '".$profildata['tmp1']."', tmp2 = '".$profildata['tmp2']."', lastlogin = NOW() WHERE loginid =".$userdata['user_id'];
