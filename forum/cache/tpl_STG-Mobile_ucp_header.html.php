...
<table cellspacing="0">
	...
	<tr>
		<td class="row1"><input class="post" type="text" name="username_list" size="20" value="" />&nbsp;<input class="post" type="submit" name="add_to" value="<?php echo ((isset($this->_rootref['L_ADD'])) ? $this->_rootref['L_ADD'] : ((isset($user->lang['ADD'])) ? $user->lang['ADD'] : '{ ADD }')); ?>" /></td>
	</tr>
</table>
	<?php } else { ?>
...
<table cellspacing="0">
	<tr>
		<td class="row1" align="center"><b class="genmed"><?php echo ((isset($this->_rootref['L_USERNAMES'])) ? $this->_rootref['L_USERNAMES'] : ((isset($user->lang['USERNAMES'])) ? $user->lang['USERNAMES'] : '{ USERNAMES }')); ?>:</b><br /><textarea name="username_list" rows="3" cols="22"></textarea><br />[ <a href="<?php echo (isset($this->_rootref['U_FIND_USERNAME'])) ? $this->_rootref['U_FIND_USERNAME'] : ''; ?>" onclick="find_username(this.href); return false;"><?php echo ((isset($this->_rootref['L_FIND_USERNAME'])) ? $this->_rootref['L_FIND_USERNAME'] : ((isset($user->lang['FIND_USERNAME'])) ? $user->lang['FIND_USERNAME'] : '{ FIND_USERNAME }')); ?></a> ]</td>
	</tr>
</table>
...
<table cellspacing="0">
		<tr>
			<td class="row1" align="center"><b class="genmed"><?php echo ((isset($this->_rootref['L_USERGROUPS'])) ? $this->_rootref['L_USERGROUPS'] : ((isset($user->lang['USERGROUPS'])) ? $user->lang['USERGROUPS'] : '{ USERGROUPS }')); ?>:</b><br /><select name="group_list[]" multiple="multiple" size="5" style="width:150px"><?php echo (isset($this->_rootref['S_GROUP_OPTIONS'])) ? $this->_rootref['S_GROUP_OPTIONS'] : ''; ?></select></td>
		</tr>
</table>
	<?php } if ($this->_rootref['S_ALLOW_MASS_PM']) {  ?>
...
<table cellspacing="0">
	<tr>
		<?php if ($_l_block1_val['S_SELECTED']) {  ?>
			...
			<?php if ($this->_rootref['S_PRIVMSGS']) {  ?><!-- the ! at the beginning of the loop name forces the loop to be not a nested one of l_block1 (it gets parsed separately) --><?php $_folder_count = (isset($this->_tpldata['folder'])) ? sizeof($this->_tpldata['folder']) : 0;if ($_folder_count) {for ($_folder_i = 0; $_folder_i < $_folder_count; ++$_folder_i){$_folder_val = &$this->_tpldata['folder'][$_folder_i]; if ($_folder_val['S_FIRST_ROW']) {  ?>
					...
					<?php } if ($_folder_val['S_CUR_FOLDER']) {  ?>
						...
						<li class="row3" style="padding: 1px 0;border-width: 1px;border-style: solid;border-color: #888888;">&#187; <a href="<?php echo $_folder_val['U_FOLDER']; ?>"><?php echo $_folder_val['FOLDER_NAME']; if ($_folder_val['S_UNREAD_MESSAGES']) {  ?> (<?php echo $_folder_val['UNREAD_MESSAGES']; ?>)<?php } ?></a></li>
					<?php } else { ?>
		...
		<?php } else { ?>
			...
			<td class="row2" nowrap="nowrap" onmouseover="this.className='row3'" onmouseout="this.className='row2'" onclick="location.href=this.firstChild.href;"><a href="<?php echo $_l_block1_val['U_TITLE']; ?>"><?php echo $_l_block1_val['L_TITLE']; ?></a></td>
		<?php } ?>
...
<table cellspacing="0">
		...
		<?php if (! $_pm_colour_info_val['IMG']) {  ?>
			...
			<td class="<?php echo $_pm_colour_info_val['CLASS']; ?>" width="10"><img src="images/spacer.gif" width="10" alt="<?php echo $_pm_colour_info_val['LANG']; ?>" /></td>
			<td class="row1" width="100%" align="center"><span class="genmed"><?php echo $_pm_colour_info_val['LANG']; ?></span></td>
		</tr>
</table>
		<?php } else { ?>
...
<form name="ucp" id="ucp" method="post" action="<?php echo (isset($this->_rootref['S_UCP_ACTION'])) ? $this->_rootref['S_UCP_ACTION'] : ''; ?>"<?php echo (isset($this->_rootref['S_FORM_ENCTYPE'])) ? $this->_rootref['S_FORM_ENCTYPE'] : ''; ?>>
<?php } ?>